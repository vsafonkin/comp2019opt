...
constexpr array<uint8_t, 14> DISTRIBUTION{2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 3, 4}; // You can setup how many families you need for swaps and what best choice use for each family
...
void init_data() {
    ifstream in("./data/family_data.csv");
...
void save_sub(const array<uint8_t, 5000>& assigned_day) {
    ofstream out("./temp/submission.csv");
        ...
        for (int k = 0; k < BEST_N; ++k) {
            ...
            for (const auto& change : changes) {
                ...
                if (score < best_local_score) {
                    printf("BINGO!!! new score %f", score);
                    found_better = true;
...
int main() {
    init_data();
    auto assigned_day = read_submission("/home/travis/.cache/pip/submission.csv");
...
// g++ -pthread -lpthread -O3 -std=c++1z -o main main.cpp